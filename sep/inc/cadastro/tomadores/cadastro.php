...
<?php	
	...
	if(($_POST['CODTOMADOR'])){		   
		...
		$sql_tomador = mysql_query("SELECT codigo,codtipo, nome, IF(cnpj <> '',cnpj,cpf) AS cnpjcpf, inscrmunicipal, logradouro, numero, cep, municipio, uf, email FROM cadastro WHERE codigo = '$codigo'");
		list($codigo,$codtipo,$nome,$cnpjcpf,$inscrmunicipal,$logradouro,$numero,$cep,$municipio,$uf,$email) = mysql_fetch_array($sql_tomador);
	}//fim if
?>
